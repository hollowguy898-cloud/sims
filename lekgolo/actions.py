...
The action space is a mixed discrete-continuous space.
We represent it as:
  - discrete action choice (0-5)
  - continuous parameters depending on the action type

Action vector layout:
  [0]    - discrete action (0-5)
  [1]    - move_dx or attach_target_idx or detach_target_idx or transfer_target_idx or attack_target_idx or unused
  [2]    - move_dy or transfer_amount or signal_0
  [3]    - signal_1
  [4]    - signal_2
  [5]    - signal_3
"""
...
ACTION_VECTOR_DIM = 1 + ACTION_PARAM_DIM  # total action vector length
...
def decode_action(action_vector: np.ndarray):
    ...
    action_vector: shape (ACTION_VECTOR_DIM,)
      [0] = discrete action type (will be argmaxed if continuous)
      [1:6] = continuous parameters
    """
...
def execute_move(worm, dx: float, dy: float, terrain: np.ndarray,
    ...
    Returns True if the worm actually moved.
    """
    if not worm.alive or not worm.spend_energy(WORM_MOVE_COST):
        return False

    # Apply thinker efficiency boost
    ...
    # Limit step size to 1.0 (one grid cell per step)
    ...
    # Apply movement cost with terrain multiplier
    actual_cost *= move_mult
    if worm.energy + actual_cost - WORM_MOVE_COST > 0:
        worm.spend_energy(max(0, actual_cost - WORM_MOVE_COST))  # extra terrain cost

    # Check attachment tether - attached worms can't move too far from partners
    for nid in worm.attachments:
        if nid in worms_by_id and worms_by_id[nid].alive:
            partner = worms_by_id[nid]
            new_dist = np.sqrt((new_x - partner.x) ** 2 + (new_y - partner.y) ** 2)
            if new_dist > ATTACHMENT_MAX_DISTANCE * 1.5:
                # Can't move that far - pull back
                direction_x = new_x - worm.x
                direction_y = new_y - worm.y
                # Reduce step to stay within tether
                max_allowed = ATTACHMENT_MAX_DISTANCE * 1.5 - worm.distance_to(partner.x, partner.y)
                if max_allowed > 0:
    ...
    if abs(norm_dx) > 1e-6 or abs(norm_dy) > 1e-6:
        worm.orientation = np.arctan2(norm_dy, norm_dx)
...
def execute_attach(worm, target_idx: int, all_worms: list,
    ...
    target_idx indexes into the worm's nearby_worms list.
    """
    ...
    d = worm.distance_to(target.x, target.y)
    if d > ATTACHMENT_MAX_DISTANCE:
        return False
...
def execute_detach(worm, target_idx: int, attachment_system,
    ...
    target_idx indexes into the worm's attachment set.
    """
    if not worm.alive:
        return False
    ...
    # Small health cost for detachment
...
def execute_transfer(worm, target_idx: int, amount: float,
    ...
    Some energy is lost in transfer (WORM_TRANSFER_COST fraction).
    """
    if not worm.alive:
        return False

    amount = float(np.clip(amount, 0, 50))  # cap transfer at 50 energy

    # Find target - prefer attached, then nearby
    # target_idx could refer to attached worms or nearby worms
    # We'll use nearby_worms list
    if not worm.nearby_worms and not worm.attachments:
        return False

    # Combine attached and nearby into a single list for indexing
    candidates = []
    ...
    target.energy = min(target.energy + actual_transfer, 200.0)  # WORM_MAX_ENERGY
...
def execute_attack(worm, target_idx: int, flood_list: list,
                   thinker_boost: dict) -> bool:
    """
    Attack a nearby Flood organism.
    """
    if not worm.alive:
        return False
    ...
    # Compute damage with thinker boost to accuracy
    base_damage = worm.attack_damage
    # Accuracy: chance to land full damage vs partial
    accuracy = 0.7 + thinker_boost.get('attack_accuracy', 0.0)
    if np.random.random() < accuracy:
        damage = base_damage
    else:
        damage = base_damage * 0.3  # glancing blow
...
def execute_signal(worm, signal_values: np.ndarray) -> bool:
    ...
    The meaning of the signal is NOT predefined - the network learns it.
    """
    if not worm.alive:
        return False
    ...
    # Clamp signal values
    worm.signal = np.clip(signal_values[:SIGNAL_DIM], -1.0, 1.0).astype(np.float32)
...
def process_action(worm, action_dict: dict, all_worms: list,
    ...
    elif action_type == 1:  # Attach
        return execute_attach(worm, params[0], all_worms, attachment_system)
    elif action_type == 2:  # Detach
        return execute_detach(worm, params[0], attachment_system, worms_by_id)
    elif action_type == 3:  # Transfer
        return execute_transfer(worm, params[0], params[1] * 50,  # scale param to energy
                                all_worms, worms_by_id)
    elif action_type == 4:  # Attack
        return execute_attack(worm, params[0], flood_list, thinker_boost)
